...
if __name__ == '__main__':
    dataset = frameDataset(Wildtrack('/home/qalam001/Data/Wildtrack'))
    multi = 10
    freq = 40
    for cam in range(dataset.num_cam):
        img = Image.open(f'/home/qalam001/Data/Wildtrack/Image_subsets/C{cam + 1}/00000025.png')
        world_img = T.ToTensor()(img).unsqueeze(0)
        img_mask = torch.ones_like(world_img)
        world_img = kornia.geometry.warp_perspective(world_img, dataset.world_from_img[[cam]], dataset.worldgrid_shape,
                                            align_corners=False)[0]
        world_mask = kornia.geometry.warp_perspective(img_mask, dataset.world_from_img[[cam]], dataset.worldgrid_shape,
        ...
        img_mask_conv1 = kornia.geometry.warp_perspective(world_mask_conv1, dataset.img_from_world[[cam]], dataset.img_shape,
                                                 align_corners=False)[0, 0].bool().numpy()
        img_mask_conv2 = kornia.geometry.warp_perspective(world_mask_conv2, dataset.img_from_world[[cam]], dataset.img_shape,
                                                 align_corners=False)[0, 0].bool().numpy()
