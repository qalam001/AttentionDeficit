...
from AttDefMVDeTr.multiview_detector.loss import *
from AttDefMVDeTr.multiview_detector.evaluation.evaluate import evaluate
...
class PerspectiveTrainer(BaseTrainer):
    ...
    def train(self, epoch, dataloader, optimizer, scaler, scheduler=None, log_interval=100):
        ...
        t_forward = 0
        t_backward = 0
        for batch_idx, (data, world_gt, imgs_gt, affine_mats, frame) in enumerate(tqdm(dataloader)):
    ...
    def visualize_world_hmap(self, batch_idx, world_gt, world_dt):
        ...
        fig, ax = plt.subplots()
        ax.imshow(world_gt['heatmap'].squeeze())
        fig.savefig(gt_dir / Path(f'{batch_idx}.png'))
        plt.close(fig)

        fig, ax = plt.subplots()
        ax.imshow(world_dt.cpu().detach().numpy().squeeze())
        fig.savefig(dt_dir / Path(f'{batch_idx}.png'))
        plt.close(fig)

    def visualize_imgs_heatmap(self, batch_idx, data, imgs_dt):
        dt_dir = self.logdir / Path(f'imgs_hmap/dt/{batch_idx}')
        dt_dir.mkdir(parents=True, exist_ok=True)

        for cam in range(7):
            hmap = imgs_dt[cam].detach().cpu().numpy().squeeze()
            img = self.denormalize(data[0, cam]).detach().cpu().numpy().squeeze().transpose([1, 2, 0])
            img = Image.fromarray((img * 255).astype('uint8'))
            overlayed = add_heatmap_to_image(hmap, img)
            overlayed.save(dt_dir / Path(f'cam_{cam}.png'))

    def test(self, epoch, dataloader, res_fpath=None, visualize=False, args=None):
        before, after, stat_dir = None, None, None
        self.model.eval()
        losses = 0
        res_list = []
        t0 = time.time()
        for batch_idx, (data, world_gt, imgs_gt, affine_mats, frame) in enumerate(tqdm(dataloader)):
            ...
            data = data.cuda()
            if args is not None:
                # add delta to 'cams' views
                masked_delta, stat_dir = args
                before = data.clone()
        ...
        #     plt.savefig(os.path.join(self.logdir, f'world{epoch if epoch else ""}.png'))
        #     plt.close(fig)
        #     # visualizing the heatmap for per-view estimation
        #     heatmap0_foot = imgs_heatmap[0].detach().cpu().numpy().squeeze()
        #     img0 = self.denormalize(data[0, 0]).detach().cpu().numpy().squeeze().transpose([1, 2, 0])
        #     img0 = Image.fromarray((img0 * 255).astype('uint8'))
        #     foot_cam_result = add_heatmap_to_image(heatmap0_foot, img0)
        #     foot_cam_result.save(os.path.join(self.logdir, 'cam1_foot.png'))
        ...
        if res_fpath is not None:
            ...
            if stat_dir is not None:
                out_str = (f'>> loss: {losses / len(dataloader):.6f}, moda: {moda:.1f}%, modp: {modp:.1f}%, '
                           f'prec: {precision:.1f}%, recall: {recall:.1f}%\n')
                with open(stat_dir, 'a') as file:
                    file.write(out_str)
                print()
                print(out_str)
        else:
