...
import kornia
from AttDefMVDeTr.multiview_detector.models.resnet import resnet18
from AttDefMVDeTr.multiview_detector.utils.image_utils import img_color_denormalize, array2heatmap
from AttDefMVDeTr.multiview_detector.utils.projection import get_worldcoord_from_imgcoord_mat, project_2d_points
from AttDefMVDeTr.multiview_detector.models.conv_world_feat import ConvWorldFeat, DeformConvWorldFeat
from AttDefMVDeTr.multiview_detector.models.trans_world_feat import TransformerWorldFeat, DeformTransWorldFeat, \
...
class MVDeTr(nn.Module):
    def __init__(self, dataset, arch='resnet18', z=0, world_feat_arch='deform_trans',
                 num_layers=3, n_heads=8, n_points=4,
        ...
        if world_feat_arch == 'conv':
            ...
            self.world_feat = DeformTransWorldFeat(dataset.num_cam, dataset.Rworld_shape, base_dim,
                                                   num_layers=num_layers, n_heads=n_heads, n_points=n_points,
                                                   stride=2, reference_points=reference_points)
    ...
    def forward(self, imgs, M, visualize=False):
        ...
        if visualize:
            ...
            proj_imgs = kornia.geometry.warp_perspective(T.Resize(self.Rimg_shape)(imgs), proj_mats.to(imgs.device),
        ...
        world_feat = kornia.geometry.warp_perspective(imgs_feat, proj_mats.to(imgs.device), self.Rworld_shape,
        ...
        if visualize:
            for cam in range(N):
                ...
                plt.imshow(visualize_img)
                plt.show()
        world_feat, atts, ptrs = self.world_feat(world_feat, visualize=visualize)
        ...
        if visualize:
            ...
            plt.imshow(visualize_img)
            plt.show()
        return (world_heatmap, world_offset), (imgs_heatmap, imgs_offset, imgs_wh), (atts, ptrs)


def test():
    from AttDefMVDeTr.multiview_detector.datasets.frameDataset import frameDataset
    from AttDefMVDeTr.multiview_detector.datasets.Wildtrack import Wildtrack
    from torch.utils.data import DataLoader
    from AttDefMVDeTr.multiview_detector.utils.decode import ctdet_decode
