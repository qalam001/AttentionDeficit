...
def main(args):
    ...
    if 'wildtrack' in args.dataset:
        base = Wildtrack(os.path.expanduser('~/wildtrack'))
    elif 'multiviewx' in args.dataset:
        base = MultiviewX(os.path.expanduser('~/multiviewX'))
    else:
    ...
    if args.resume is None:
        logdir = f'logs/{args.dataset}/{"debug_" if is_debug else ""}{"SS_" if args.semi_supervised else ""}' \
                 f'{"aug_" if args.augmentation else ""}{args.world_feat}_nL{args.num_layers}_lr{args.lr}_baseR{args.base_lr_ratio}_' \
    ...
    # model
    model = MVDeTr(train_set, args.arch, num_layers=args.num_layers, world_feat_arch=args.world_feat,
